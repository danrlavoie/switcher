 ...
 */
function getLineFromLines(lines) {
  if(lines.length > 1) {
    return lines.split('\r\n')[3];
  } else {
    return lines;
 ...
 * @param telnetParams The parameters required to connect to the telnet server. Hostname and port.
 * @param command The command to be procssed by the telnet server.
 * @returns The response from the telnet server.
 */
export async function telnetResponse(telnetParams, command) {
  const connection = new Telnet();

  const params = {
    host: telnetParams.host,
